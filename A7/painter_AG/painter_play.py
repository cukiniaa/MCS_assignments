...
# %Code translated into python Fiona Skerman from matlab code written by Alex
# Szorkovszky for UU Modelling Complex Systems

# %INPUTS
# %rules: 54-cell array with one of three actions: 0(no turn) 1(turn left)
# 2(turn right) 3(random turn left/right)
# %room: MxN matrix defining a rectangular room with each square either
# 0(empty) or 1(furniture) or 2(painted)


# Chromosome (54-cell rule array) encodes action for each of 54 different
# scenarios
# no turn, turn left, turn right, turn left/right with 50/50
# let [c, f, l, r] denote the states of current, forward, left and right
# squares,
# then rule for that position is at position i=2(9f+3l+r)+Indicator[c=2] in
# chromosone.


# Painter has a position x,y in MxN matrix, and a direction
# -1 Left, 0 Up, 1 Right, -2 Down.

# Each time step consists of three parts
# a) according to rule on current environment update direction either 0 no turn
# 1 turn left, 2 turn right, 3 random turn left/right
...
def painter_play(rules, room):
    ...
    for i in range(t):
        ...
        # dx, dy of a square to right (given current direction)
        ...
        local = [env[xpos[i] + dx, ypos[i] + dy],
                 env[xpos[i] - dxr, ypos[i] - dyr],
                 env[xpos[i] + dxr, ypos[i] + dyr]]

        # localnum= 2* np.dot([9,3,1], local) if env[xpos[i], ypos[i]] == 2
        # else 2* np.dot([9,3,1], local) + 1
        localnum = int(2 * np.dot([9, 3, 1], local))
        if env[xpos[i], ypos[i]] == 2:
            localnum += 1

        # use turning rule 1 'turn left', 2 'turn right', 3 'turn left/right
        # 50/50 probabilities'
        if rules[localnum] == 3:
